...
function processRequestData(\Pinga\Auth\Auth $auth) {
	if (isset($_POST)) {
		if (isset($_POST['action'])) {
			if ($_POST['action'] === 'login') {
				...
				catch (\Pinga\Auth\InvalidEmailException $e) {
					return 'wrong email address';
				}
				catch (\Pinga\Auth\UnknownUsernameException $e) {
					return 'unknown username';
				}
				catch (\Pinga\Auth\AmbiguousUsernameException $e) {
					return 'ambiguous username';
				}
				catch (\Pinga\Auth\InvalidPasswordException $e) {
					return 'wrong password';
				}
				catch (\Pinga\Auth\EmailNotVerifiedException $e) {
				...
				catch (\Pinga\Auth\UserAlreadyExistsException $e) {
					return 'email address already exists';
				}
				catch (\Pinga\Auth\DuplicateUsernameException $e) {
				...
				catch (\Pinga\Auth\InvalidSelectorTokenPairException $e) {
					return 'invalid token';
				}
				catch (\Pinga\Auth\TokenExpiredException $e) {
				...
				catch (\Pinga\Auth\ConfirmationRequestNotFound $e) {
				...
				catch (\Pinga\Auth\ResetDisabledException $e) {
				...
				if (isset($_POST['id'])) {
					...
					catch (\Pinga\Auth\UnknownIdException $e) {
						return 'unknown ID';
					}
				}
...
function showDebugData(\Pinga\Auth\Auth $auth, $result) {
	echo '<pre>';
	...
	\var_dump($auth->hasRole(\Pinga\Auth\Role::SUPER_MODERATOR));

	echo 'Roles (developer *or* manager)' . "\t\t";
	\var_dump($auth->hasAnyRole(\Pinga\Auth\Role::DEVELOPER, \Pinga\Auth\Role::MANAGER));

	echo 'Roles (developer *and* manager)' . "\t\t";
	\var_dump($auth->hasAllRoles(\Pinga\Auth\Role::DEVELOPER, \Pinga\Auth\Role::MANAGER));
	...
	\var_dump(\Pinga\Auth\Auth::createRememberCookieName());
	echo "\n";

	echo 'Auth::createCookieName(\'session\')' . "\t";
	\var_dump(\Pinga\Auth\Auth::createCookieName('session'));
	echo 'Auth::createRandomString()' . "\t\t";
	\var_dump(\Pinga\Auth\Auth::createRandomString());
	echo 'Auth::createUuid()' . "\t\t\t";
	\var_dump(\Pinga\Auth\Auth::createUuid());

	echo '</pre>';
}

function convertStatusToText(\Pinga\Auth\Auth $auth) {
	if ($auth->isLoggedIn() === true) {
		if ($auth->getStatus() === \Pinga\Auth\Status::NORMAL && $auth->isNormal()) {
			return 'normal';
		}
		elseif ($auth->getStatus() === \Pinga\Auth\Status::ARCHIVED && $auth->isArchived()) {
			return 'archived';
		}
		elseif ($auth->getStatus() === \Pinga\Auth\Status::BANNED && $auth->isBanned()) {
			return 'banned';
		}
		elseif ($auth->getStatus() === \Pinga\Auth\Status::LOCKED && $auth->isLocked()) {
			return 'locked';
		}
		elseif ($auth->getStatus() === \Pinga\Auth\Status::PENDING_REVIEW && $auth->isPendingReview()) {
			return 'pending review';
		}
		elseif ($auth->getStatus() === \Pinga\Auth\Status::SUSPENDED && $auth->isSuspended()) {
			return 'suspended';
		}
	}
...
function showAuthenticatedUserForm(\Pinga\Auth\Auth $auth) {
...
function createRolesOptions() {
	...
	foreach (\Pinga\Auth\Role::getMap() as $roleValue => $roleName) {
